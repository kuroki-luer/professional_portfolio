...
) const
{
    ...
    forAll(this->reactions(), i)
    {
        if (!reactionsDisabled_[i])
        {
            ...
            (
                R, c2, T, p, Te, Tion, pf, cf, lRef, pr, cr, rRef
            );
...
template<class CompType, class ThermoType>
Foam::tmp<Foam::volScalarField>
Foam::TDACChemistryModel<CompType, ThermoType>::eChemSourceExplicit() const
{
    scalar pf, cf, pr, cr;
    label lRef, rRef;
...
void Foam::TDACChemistryModel<CompType, ThermoType>::calculate()
{
    ...
    if (this->chemistry_)
    {
        forAll(rho, celli)
        {
            ...
            // add new code in here from modified TDAC chemistry model
            scalar pf, cf, pr, cr;
            label lRef, rRef;
            ...
            // end of code added from modified TDAC chemistry model 
...
template<class CompType, class ThermoType>
void Foam::TDACChemistryModel<CompType, ThermoType>::calculateWcf()
{
    const volScalarField rho
    (
        IOobject
...
) const
{
    ...
    scalar Te = c[this->nSpecie() + 2];
    scalar Tion = c[this->nSpecie() + 3];

    //the size of dcdt is c.size() (i.e. speciesNumber+2)
    scalarField tdcdt(omega(c, T, p, Te, Tion));
    forAll(tdcdt, i)
    {
        dcdt[i] = tdcdt[i];
    }
    ...
    dcdt[this->nSpecie()] = -dT*dtMag/(mag(dT) + SMALL);
