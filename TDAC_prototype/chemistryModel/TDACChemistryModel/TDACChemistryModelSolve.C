...
*/
        ...
        phiq[this->nSpecie()+2]=Tei;
        phiq[this->nSpecie()+3]=Tioni;
...
//                updateTreeCpuTime_ += clockTime_.timeIncrement();             
        ...
        {
clockTime_.timeIncrement();
	    if (DAC_) mechRed_->reduceMechanism(c, Ti, pi, Tei, Tioni);
		...
		if (DAC_)
		    ...
		    completeC_ = c;
                    tauC = this->solver().solve(simplifiedC_, Ti, pi, Tei, Tioni, t, dt);
                ...
                else
                {
                    ...
                    //the value of c is updated in the solve function of the chemistrySolverTDAC
                    tauC = this->solver().solve(c, Ti, pi, Tei, Tioni, t, dt);
                }
		t += dt;
...
//------TEST------//            
                ...
                scalar rhoi = rho[tmpCelli];
                Ti=this->thermo().T()[tmpCelli];
                pi=this->thermo().p()[tmpCelli];
                hi=this->thermo().hs()[tmpCelli] + hc[tmpCelli];
                Tei=this->thermo().Te()[tmpCelli];
                Tioni=this->thermo().Tion()[tmpCelli];

                phiq[this->nSpecie()]=Ti;
                phiq[this->nSpecie()+1]=pi;
                phiq[this->nSpecie()+2]=Tei;
                phiq[this->nSpecie()+3]=Tioni;
                // I think we need to add the other things anyway
                ...
                if(!retrieved)
                {
                    ...
                    // this corresponds to te while loop in ODEChemistryModel at line 1045
                    while(timeLeft > SMALL)
                    {
                        if (DAC_)
                        {
...
*/
                    ...
                    else
                    {
                        ...
                        Rcq[this->nSpecie()+2]=Tei;
                        Rcq[this->nSpecie()+3]=Tioni;
...
) const
{
    ...
    scalar Te = c2[this->nSpecie() + 2];
    scalar Tion = c2[this->nSpecie() + 3];

    for(label i=0; i<speciesNumber+4; i++)
    {
        for(label j=0; j<speciesNumber+4; j++)
        {
            dfdc[i][j] = 0.0;
        }
    }
